...
/**
 *
 * --> canonical not working, show same customized url for all languages
 *
 */
...
const pagesCompile = () => {
  ...
  const pagesTasks = pages.map((pagePath) => {
    ...
    pageLanguages.forEach((pageLanguage, pageLanguageIndex) => {
      ...
      const linkAlternate = pageLanguages.map((altLanguage) => {
        return {
          lang: altLanguage,
  ...
  const sitemapTask = (gulp.src(`${config.sourcePath}partials/sitemap.hbs`)
